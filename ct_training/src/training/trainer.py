...
from config.ct_config import (
    ...
    METRIC_CHECK_START_EPOCH, MODEL_SAVE_PATH, 
    RANDOM_SEED, BATCH_SIZE
)

class CTTrainer:
    """
    Core class for training the CT model using five-fold cross-validation.
    ...
    def _save_checkpoint(self, fold_idx: int, epoch: int, metrics: dict, is_best: bool):
        ...
        if is_best:
            old_best_files = [f for f in os.listdir(MODEL_SAVE_PATH) if f.startswith(f"{filename}_best_model")]
            ...
            old_epoch_files = [f for f in os.listdir(MODEL_SAVE_PATH) if f.startswith(f"{filename}_epoch")]
            for old_file in old_epoch_files:
                os.remove(os.path.join(MODEL_SAVE_PATH, old_file))
                
            save_path = os.path.join(MODEL_SAVE_PATH, f"{filename}_epoch_{epoch + 1}.pth")
    ...
    def run_fold(
        ...
        self.best_metric = -float('inf')
        self.epochs_no_improve = 0
        self.model.apply(lambda m: self.model.init_weights() if hasattr(m, 'init_weights') else None) 
